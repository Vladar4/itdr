...
\subparagraph{Mystic} rolls for random Cantrips (d20) and \nth{1} Circle Spells (\textbf{Random 36 Spells} table from \textbf{\fullref{ch:magic}}\safepageref{, page }{random_spells}{}). Choose one of these spells as a Signature Spell or roll a d6 for a random one.
...
\header{Simple Melee Weapons (1s)}
\begin{dtable}[cL|cL]
...
\header{Martial Melee Weapons (10s)}
\begin{dtable}[cL|cL]
...
\header{Simple Ranged Weapons (1s)}
\begin{dtable}[cL|cL]
...
\header{Martial Ranged Weapons (10s)}
\begin{dtable}[cL|cL]
	\textbf{d4} & \textbf{Weapon} & \textbf{d4} & \textbf{Weapon} \\
	1 & crossbow	& 3 & musket \\
	2 & longbow		& 4 & pistol \\
...
Roll d8 die d4 times to choose which tables to use.
\begin{itemize}
	\item Reroll duplicates.
	\item If the \textbf{Form} table was not chosen, use it anyway.
	\item If you got the \textbf{Form} table only, roll once more.
\end{itemize}
